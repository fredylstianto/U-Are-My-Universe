...
class AppController {
  ...
  async init() {
    try {
      ...
      // 3.5 Passcode (runs before preloader)
      if (window.PasscodeSystem && CONFIG.passcode?.enabled !== false) {
        this.systems.passcode = new PasscodeSystem();
        await this.systems.passcode.run();
      }
